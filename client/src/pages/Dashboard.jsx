    ...
    return (
      <Layout>
        <div className="flex justify-between items-center mb-6">
          ...
          <button onClick={fetchData} className="px-3 py-1.5 text-sm rounded-lg bg-white/10 dark:bg-black/10 backdrop-blur-md border border-white/20 text-indigo-600 dark:text-indigo-400 hover:bg-white/20 dark:hover:bg-black/20">🔄 Refresh</button>
        </div>
            ...
            {data?.enrolledCourses?.length ? (
              <ul className="space-y-2">
                {data.enrolledCourses.map((e) => (
                  <li key={e.id} className="p-4 rounded-lg bg-white/10 dark:bg-black/10 backdrop-blur-md border border-white/20">
      ...
      <Layout>
        ...
        {user.role === 'admin' && data?.users && data.users.length > 0 && (
          <div className="mt-8">
            ...
            <ul className="space-y-2">
              {data.users.map((u) => (
                <li key={u.id} className="p-4 rounded-lg bg-white/10 dark:bg-black/10 backdrop-blur-md border border-white/20">
